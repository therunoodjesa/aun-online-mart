...
function ProductEditor({ visible, productId, products, onClose, onSaved }: { visible: boolean; productId: string | null; products: Product[]; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    if (!name.trim() || Number(price) <= 0 || !categories.length) { Alert.alert('Complete the item', 'Add a name, a price above zero, and at least one service period.'); return; }
    ...
    setSaving(true);
    const payload = { name: name.trim(), description: description.trim() || null, price: Number(price), stock_quantity: stock.trim() ? Math.max(0, Number(stock) || 0) : null, category: categories[0], categories, meal_plan_eligible: mealEligible, image_url: imageUrl.trim() || null };
