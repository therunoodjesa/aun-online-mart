...
export default function CafeteriaPortalWorkspace() {
  ...
  const navigation = <><Nav section={section} id="overview" label="Overview" icon="grid-outline" onPress={setSection} />
  ...
  return <View style={styles.screen}>
    ...
    <View style={[styles.topbar, compact && styles.topbarCompact]}>
      <View style={styles.brand}><Ionicons name="restaurant-outline" size={compact ? 22 : 25} color="#68ECCB" /><Text style={[styles.brandName, compact && styles.brandNameCompact]}>AUN Cafeteria</Text>{!compact ? <Text style={styles.brandPortal}>Operations portal</Text> : null}</View>
      <View style={[styles.topActions, compact && styles.topActionsCompact]}><View style={[styles.acceptingPill, compact && styles.acceptingPillCompact]}><View style={[styles.liveDot, !settings.is_accepting_orders && styles.closedDot]} /><Text style={styles.acceptingText}>{settings.is_accepting_orders ? (compact ? 'Open' : 'Accepting orders') : (compact ? 'Paused' : 'Orders paused')}</Text></View><View style={[styles.rolePill, compact && styles.rolePillCompact]}><Ionicons name="shield-checkmark-outline" size={18} color="#68ECCB" />{!compact ? <Text style={styles.roleText}>{roleNames[staff.role]}</Text> : null}</View></View>
    </View>
    {compact ? <ScrollView horizontal style={styles.mobileNav} contentContainerStyle={styles.mobileNavContent} showsHorizontalScrollIndicator={false}>{navigation}<TouchableOpacity style={styles.mobileLogout} onPress={() => void signOut()}><Ionicons name="log-out-outline" size={18} color="#B44646" /><Text style={styles.mobileLogoutText}>Log out</Text></TouchableOpacity></ScrollView> : null}
    <View style={[styles.body, compact && styles.bodyCompact]}>
      ...
      <ScrollView style={styles.workspace} contentContainerStyle={[styles.workspaceContent, compact && styles.workspaceContentCompact]} showsVerticalScrollIndicator>
  ...
  return <Modal transparent visible={visible} animationType="fade" onRequestClose={onClose}>
    <View style={styles.modalBackdrop}>
      <ScrollView style={styles.modal} contentContainerStyle={styles.modalContent} showsVerticalScrollIndicator={false}>
        <Text style={styles.modalTitle}>{productId ? 'Edit cafeteria item' : 'Add cafeteria item'}</Text>
        <Field label="Item name" value={name} onChangeText={setName} placeholder="e.g. Jollof rice" />
        <Field label="Description (optional)" value={description} onChangeText={setDescription} placeholder="What the customer should know" multiline large />
        <View style={styles.fieldPair}>
          <Field label="Price (NGN — ₦0 allowed for included add-ons)" value={price} onChangeText={setPrice} placeholder="1800" keyboardType="numeric" half />
          <Field label="Stock (blank = unlimited)" value={stock} onChangeText={setStock} placeholder="Unlimited" keyboardType="numeric" half />
        </View>
        <Text style={styles.fieldLabel}>Product image</Text>
        <TouchableOpacity style={styles.upload} onPress={() => void chooseImage()} disabled={uploading}>{uploading ? <ActivityIndicator color="#176E73" /> : <><Ionicons name="image-outline" size={19} color="#176E73" /><Text style={styles.uploadText}>{imageUrl ? 'Replace uploaded image' : 'Upload from device'}</Text></>}</TouchableOpacity>
        <Field label="Or paste an image URL" value={imageUrl} onChangeText={setImageUrl} placeholder="https://..." />
        {imageUrl ? <Image source={{ uri: imageUrl }} style={styles.editorPreview} /> : null}
        <Text style={styles.fieldLabel}>Show item in</Text>
        <View style={styles.choicePills}>{periods.map((period) => <TouchableOpacity key={period} onPress={() => setCategories((current) => current.includes(period) ? current.filter((value) => value !== period) : [...current, period])} style={[styles.choicePill, categories.includes(period) && styles.choicePillActive]}><Ionicons name={categories.includes(period) ? 'checkmark-circle' : 'ellipse-outline'} size={16} color="#176E73" /><Text style={styles.choicePillText}>{period}</Text></TouchableOpacity>)}</View>
        <TouchableOpacity style={[styles.eligible, mealEligible && styles.eligibleActive]} onPress={() => setMealEligible((current) => !current)}><Ionicons name={mealEligible ? 'checkmark-circle' : 'ellipse-outline'} size={19} color="#176E73" /><Text style={styles.eligibleText}>Eligible for meal-plan credit</Text></TouchableOpacity>
        <Text style={[styles.fieldLabel, { marginTop: 18 }]}>Customer choices</Text>
        <Text style={styles.help}>Use separate groups such as Protein and Side. Customers can choose one option from each group.</Text>
        {choices.map((choice, index) => <View key={index} style={styles.choiceEditorRow}>
          <TextInput value={choice.group} onChangeText={(group) => setChoices((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, group } : row))} placeholder="Group" placeholderTextColor="#98A2AE" style={styles.choiceInput} />
          <TextInput value={choice.name} onChangeText={(optionName) => setChoices((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, name: optionName } : row))} placeholder="Option name" placeholderTextColor="#98A2AE" style={[styles.choiceInput, { flex: 1.25 }]} />
          <TextInput value={choice.price} onChangeText={(choicePrice) => setChoices((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, price: choicePrice } : row))} placeholder="Extra ₦" placeholderTextColor="#98A2AE" keyboardType="numeric" style={[styles.choiceInput, { maxWidth: 92 }]} />
          <TouchableOpacity style={styles.removeChoice} onPress={() => setChoices((rows) => rows.filter((_, rowIndex) => rowIndex !== index))}><Ionicons name="trash-outline" size={17} color="#B44646" /></TouchableOpacity>
        </View>)}
        <TouchableOpacity style={styles.addChoice} onPress={() => setChoices((rows) => [...rows, { group: rows.at(-1)?.group ?? '', name: '', price: '0' }])}><Ionicons name="add-circle-outline" size={18} color="#176E73" /><Text style={styles.addChoiceText}>Add radio choice</Text></TouchableOpacity>
        <View style={styles.modalActions}><TouchableOpacity style={styles.cancelButton} onPress={onClose}><Text style={styles.cancelButtonText}>Cancel</Text></TouchableOpacity><TouchableOpacity disabled={saving || uploading} style={styles.saveButton} onPress={() => void save()}>{saving ? <ActivityIndicator color="#01193D" /> : <Text style={styles.saveButtonText}>Save item</Text>}</TouchableOpacity></View>
      </ScrollView>
    </View>
  </Modal>;
...
const styles = StyleSheet.create({
  ...
  topActionsCompact: { gap: 7 },
  acceptingPillCompact: { height: 36, paddingHorizontal: 10 },
  rolePillCompact: { width: 36, height: 36, paddingHorizontal: 0, justifyContent: 'center' },
  bodyCompact: { flexDirection: 'column' },
  mobileNav: { maxHeight: 62, borderBottomWidth: 1, borderBottomColor: '#D9DFE5', backgroundColor: '#FFF' },
  mobileNavContent: { paddingHorizontal: 8, alignItems: 'center', gap: 3 },
  mobileLogout: { minHeight: 42, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', gap: 6, marginLeft: 4 },
  mobileLogoutText: { color: '#B44646', fontSize: 13, fontWeight: '800' },
  workspaceContentCompact: { padding: 16, paddingBottom: 42 },
