-- WITH ORDINALITY returns bigint while service_options.sort_order is integer.
-- Cast explicitly so saving a service with booking choices remains atomic.
...
begin
  ...
  select v_service_id, trim(entry.value->>'name'), (entry.value->>'price')::numeric,
    ...
    coalesce((entry.value->>'is_available')::boolean, true), entry.ordinality::integer
