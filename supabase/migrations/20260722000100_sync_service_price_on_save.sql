-- The original services table has a required `price` column. The service
-- catalogue introduced `starting_price`; keep both values synchronized so
-- older constraints and newer buyer views remain compatible.
update public.services
set starting_price = coalesce(starting_price, price),
    price = coalesce(price, starting_price)
where starting_price is null or price is null;
