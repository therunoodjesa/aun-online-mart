-- Parse booking choices directly from JSON. This avoids the column-definition
-- ambiguity of jsonb_to_recordset WITH ORDINALITY when a service has options.
...
begin
  ...
  where owner_id = auth.uid() and store_type = 'service' and is_approved = true;
  ...
  if p_service_id is null then
    select coalesce(max(sort_order), 0) + 1 into v_next_sort from public.services where vendor_id = v_vendor_id;
    ...
    update public.services set name = trim(p_name), category = trim(p_category), description = nullif(trim(p_description), ''), starting_price = p_starting_price, duration_minutes = greatest(1, coalesce(p_duration_minutes, 60)), image_url = nullif(trim(p_image_url), ''), is_available = coalesce(p_is_available, true), updated_at = now()
    where id = p_service_id and vendor_id = v_vendor_id returning id into v_service_id;
  ...
  select v_service_id, trim(entry.value->>'name'), (entry.value->>'price')::numeric,
    ...
    coalesce((entry.value->>'is_available')::boolean, true), entry.ordinality
  from jsonb_array_elements(coalesce(p_options, '[]'::jsonb)) with ordinality as entry(value, ordinality)
  where coalesce(length(trim(entry.value->>'name')), 0) > 0 and coalesce((entry.value->>'price')::numeric, -1) >= 0;
  return v_service_id;
end;
$$;
