...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: order, error: orderError } = await db.from('orders')
      .select('id, user_id, order_number, payment_status, payment_reference, amount_paid, subtotal, total, delivery_type, delivery_address, delivery_instructions, delivery_slot, created_at')
    ...
    const [{ data: productItems, error: itemError }, { data: cafeteriaItems, error: cafeteriaItemError }] = await Promise.all([
      db.from('order_items').select('product_name, unit_price, quantity, total_price, options, notes').eq('order_id', order.id),
      db.from('cafeteria_order_items').select('product_name, unit_price, quantity, options, notes').eq('order_id', order.id),
    ]);
    if (itemError) throw new Error(itemError.message);
    if (cafeteriaItemError) throw new Error(cafeteriaItemError.message);
    const items = [...(productItems ?? []), ...(cafeteriaItems ?? []).map((item) => ({ ...item, total_price: Number(item.unit_price) * Number(item.quantity) }))];
    ...
    const fulfilmentDetail = order.delivery_type === 'pickup' ? 'Your order will be ready for collection at the vendor location.' : `${order.delivery_address || 'AUN delivery address'}${order.delivery_instructions ? ` · ${order.delivery_instructions}` : ''}${order.delivery_slot ? ` · ${order.delivery_slot}` : ''}`;
