...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const db = admin();
    const { data: intent, error } = await db.from('payment_intents').insert({ user_id: user.id, reference, amount_kobo: Math.round(priced.total * 100), fulfilment, delivery_address: body.address ?? null, delivery_instructions: body.delivery_instructions ?? null, delivery_slot: body.slot ?? null, cart: { ...priced, fulfilment } }).select('id').single();
    ...
    return json({ reference, authorization_url: transaction.authorization_url, amount: priced.total, pricing: priced });
