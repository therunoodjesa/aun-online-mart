...
type StoredCart = { lines?: StoredLine[]; subtotal?: number; total?: number; deliveryFee?: number; rushHour?: { savings?: number }; promoCode?: string | null; promoDiscount?: number };
...
async function finalisePaidIntent(db: ReturnType<typeof admin>, intent: { id: string; user_id: string; reference: string; order_id: string | null; fulfilment: string; delivery_address: string | null; delivery_instructions: string | null; delivery_slot: string | null; cart: StoredCart }, transactionId: number) {
  ...
  if (!orderId) {
    ...
    const { data: order, error } = await db.from('orders').insert({
      ...
      subtotal, total, delivery_fee: Number(intent.cart.deliveryFee ?? 0), rush_hour_discount: Number(intent.cart.rushHour?.savings ?? 0), promo_code: intent.cart.promoCode ?? null, promo_discount: Number(intent.cart.promoDiscount ?? 0), delivery_address: intent.delivery_address, delivery_instructions: intent.delivery_instructions, delivery_slot: intent.delivery_slot,
    }).select('id').single();
    if (error || !order) {
