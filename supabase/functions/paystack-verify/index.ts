...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const cart = intent.cart as { lines: { source?: 'marketplace' | 'cafeteria'; product_id: string | null; cafeteria_product_id?: string | null; product_name: string; unit_price: number; quantity: number; selected_options: unknown[]; note: string | null; meal_plan_credit?: number; packaging_fee?: number }[]; subtotal: number; total: number; deliveryFee?: number; rushHour?: { savings?: number }; promoCode?: string | null; promoDiscount?: number };
    const orderNumber = `AOM-${String(Date.now()).slice(-7)}`;
    const { data: order, error: orderError } = await db.from('orders').insert({ order_number: orderNumber, user_id: user.id, status: 'pending', delivery_type: intent.fulfilment, payment_status: 'pending', payment_reference: reference, amount_paid: cart.total, subtotal: cart.subtotal, total: cart.total, delivery_fee: Number(cart.deliveryFee ?? 0), rush_hour_discount: Number(cart.rushHour?.savings ?? 0), promo_code: cart.promoCode ?? null, promo_discount: Number(cart.promoDiscount ?? 0), delivery_address: intent.delivery_address, delivery_instructions: intent.delivery_instructions, delivery_slot: intent.delivery_slot }).select('id').single();
    if (orderError || !order) throw new Error(orderError?.message ?? 'Could not create your order.');
    const marketplaceLines = cart.lines.filter((line) => line.source !== 'cafeteria');
    const cafeteriaLines = cart.lines.filter((line) => line.source === 'cafeteria');
    const { error: itemError } = marketplaceLines.length ? await db.from('order_items').insert(marketplaceLines.map((line) => ({ order_id: order.id, product_id: line.product_id, product_name: line.product_name, unit_price: line.unit_price, quantity: line.quantity, total_price: line.unit_price * line.quantity, options: line.selected_options, notes: line.note }))) : { error: null };
    if (itemError) throw new Error(itemError.message);
    const { error: cafeteriaItemError } = cafeteriaLines.length ? await db.from('cafeteria_order_items').insert(cafeteriaLines.map((line) => ({ order_id: order.id, product_id: line.cafeteria_product_id, product_name: line.product_name, unit_price: line.unit_price, quantity: line.quantity, options: line.selected_options, notes: line.note, meal_plan_credit: Number(line.meal_plan_credit ?? 0), packaging_fee: Number(line.packaging_fee ?? 0) }))) : { error: null };
