...
type Option = { id?: string; option_group: string; name: string; price_modifier: string; color_hex?: string | null; is_available: boolean; selection_mode?: 'multiple' | 'single' };
type OptionSuggestion = { group: string; choices: { name: string; price_modifier: string; color_hex?: string | null }[]; selection_mode: 'multiple' | 'single' };
...
export default function CatalogueItemEditor() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: productOptions } = await supabase.from('product_options').select('id, option_group, name, price_modifier, color_hex, is_available, selection_mode').eq('product_id', id).order('option_group');
  ...
  useEffect(() => {
    const loadSuggestions = async () => {
      ...
      const { data: rows } = await supabase.from('product_options').select('option_group, name, price_modifier, color_hex, selection_mode').in('product_id', productIds).eq('is_available', true).order('option_group').order('name');
      ...
      for (const row of rows ?? []) {
        ...
        if (!suggestion.choices.some((choice) => choice.name.toLowerCase() === name.toLowerCase())) suggestion.choices.push({ name, price_modifier: String(row.price_modifier ?? 0), color_hex: row.color_hex ?? null });
  ...
  const useSuggestion = (suggestion: OptionSuggestion) => setOptions((current) => {
    ...
    const additions = suggestion.choices.filter((choice) => !existing.has(`${suggestion.group.toLowerCase()}::${choice.name.toLowerCase()}`)).map((choice) => ({ option_group: suggestion.group, name: choice.name, price_modifier: choice.price_modifier, color_hex: choice.color_hex ?? '', is_available: true, selection_mode: suggestion.selection_mode }));
  ...
  const save = async () => {
    ...
    const { data, error } = await supabase.functions.invoke('vendor-catalogue-save', { body: {
      ...
      options: options.filter((option) => option.name.trim() && option.option_group.trim()).map((option) => ({ option_group: option.option_group.trim(), name: option.name.trim(), price_modifier: Number(option.price_modifier) || 0, color_hex: option.color_hex?.trim() || null, is_available: option.is_available, selection_mode: option.selection_mode ?? 'multiple' })),
    } });
