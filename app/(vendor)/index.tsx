...
import { ActivityIndicator, Alert, FlatList, Modal, ScrollView, StyleSheet, Switch, Text, TextInput, TouchableOpacity, useWindowDimensions, View } from 'react-native';
...
const mobilePages: Page[] = ['inventory', 'orders', 'settings'];
...
export default function VendorPortal() {
  ...
  const load = async () => {
    ...
    if (authError || !auth.user) { setPortalError('Your sign-in has expired. Please sign in again to continue managing your store.'); setLoading(false); return; }
  ...
  useEffect(() => { void load(); }, []);
  useEffect(() => {
    if (compact && !mobilePages.includes(page)) setPage('inventory');
  }, [compact, page]);
  useEffect(() => {
    if (!vendor?.id) { setNewOrderCount(0); return; }
    let active = true;
    const refreshOrderNotice = async () => { const orders = await loadVendorOrders(vendor.id); if (active) setNewOrderCount(orders.filter((order) => order.status === 'pending' || order.status === 'replacement_selected').length); };
...
function InventoryMobileCard({ item, setStatus, setStock, setStockValue, moveProduct, onOpenPosition }: { item: Product; setStatus: (item: Product, value: Product['status']) => void; setStock: (item: Product, change: number) => void; setStockValue: (item: Product, value: string) => void; moveProduct: (item: Product, offset: number) => void; onOpenPosition: () => void }) {
...
function PlacementHandle({ item, onOpen }: { item: Product; onMove: (item: Product, offset: number) => void; onOpen?: () => void }) {
  return <TouchableOpacity onPress={onOpen} style={inventoryStyles.placementHandle} accessibilityRole="button" accessibilityLabel={`Set the position of ${item.name}`}><View style={inventoryStyles.handleLong} /><View style={inventoryStyles.handleShort} /></TouchableOpacity>;
}
...
function Dashboard({ vendor, products, onOrders }: { vendor: Vendor; products: Product[]; onOrders: () => void }) {
  ...
  useEffect(() => { void loadVendorOrders(vendor.id).then(setOrders); }, [vendor.id]);
...
function Orders({ vendor, compact = false }: { vendor: Vendor; compact?: boolean }) {
  const [orders, setOrders] = useState<VendorOrder[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  const load = async (showLoading = false) => { if (showLoading) setLoading(true); setOrders(await loadVendorOrders(vendor.id)); if (showLoading) setLoading(false); };
...
function Settings({ vendor, onSaved, onLogOut, compact = false }: { vendor: Vendor; onSaved: (details: Partial<Vendor>) => void; onLogOut: () => void; compact?: boolean }) {
...
function Analytics({ vendor }: { vendor: Vendor }) {
  const [orders, setOrders] = useState<VendorOrder[]>([]);
  const [range, setRange] = useState<7 | 30 | 'all'>(7);
  const [loading, setLoading] = useState(true);
  const storeName = vendor.name || 'Your store';
  const isService = vendor.store_type === 'service';
  useEffect(() => { void (async () => { setLoading(true); setOrders(await loadVendorOrders(vendor.id)); setLoading(false); })(); }, [vendor.id]);
...
const applicationGuideStyles = StyleSheet.create({
  card: { marginTop: -6, marginBottom: 18, padding: 13, borderRadius: 10, backgroundColor: '#F0F8F7', borderWidth: 1, borderColor: '#D5EEE8', gap: 7 },
  line: { color: '#526273', fontSize: 13, lineHeight: 19 },
  type: { color: '#176E73', fontWeight: '800' },
});
