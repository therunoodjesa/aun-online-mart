...
export default function VendorPortal() {
  ...
  const load = async () => {
    ...
    const { data: vendorRow } = await supabase.from('vendors').select('id, name, is_open, store_type, operating_location, description, banner_url, category, location, pickup_location, pickup_instructions, important_message, average_prep_time').eq('owner_id', auth.user.id).maybeSingle();
    if (vendorRow) {
  ...
  const setOpen = async (is_open: boolean) => {
    ...
    if (error || scheduleError) { void load(); Alert.alert('Could not update store', error?.message ?? scheduleError?.message); }
  };
  ...
  const logOut = () => Alert.alert('Log out of vendor portal?', 'You will need to sign in again to manage this store.', [
    { text: 'Cancel', style: 'cancel' },
    { text: 'Log out', style: 'destructive', onPress: () => { void (async () => { const { error } = await supabase.auth.signOut(); if (error) { Alert.alert('Could not log out', error.message); return; } router.replace('/(auth)/login'); })(); } },
  ]);
