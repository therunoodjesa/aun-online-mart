...
import { ActivityIndicator, Alert, FlatList, ScrollView, StyleSheet, Switch, Text, TextInput, TouchableOpacity, useWindowDimensions, View } from 'react-native';
  ...
  return <>
    ...
    <FlatList
      ...
      renderItem={({ item }) => <View style={styles.row}>
        ...
        <View style={{ flex: 2.2 }}><Text style={styles.itemName}>{item.name}</Text><Text style={styles.itemSub}>{item.category ?? 'Menu item'}</Text></View>
        <Text style={styles.price}>{'₦' + item.price.toLocaleString('en-NG')}</Text>
        <View style={[styles.status, item.status === 'sold_out' && styles.sold, item.status === 'hidden' && styles.hidden]}><View style={styles.statusDot} /><Text style={styles.statusText}>{item.status === 'available' ? 'Available' : item.status === 'sold_out' ? 'Sold out' : 'Hidden'}</Text></View>
...
function Dashboard({ vendor, products, onOrders }: { vendor: Vendor; products: Product[]; onOrders: () => void }) {
  ...
  const sales = orders.reduce((total, order) => total + order.items.reduce((sum, item) => sum + item.unit_price * item.quantity, 0), 0);
...
function Payouts({ vendor }: { vendor: Vendor }) {
  ...
  const available = eligibleCompleted.reduce((total, order) => total + order.items.reduce((sum, line) => sum + line.unit_price * line.quantity, 0), 0);
  const pending = orders.filter((order) => ['pending', 'accepted', 'preparing'].includes(order.status)).reduce((total, order) => total + order.items.reduce((sum, line) => sum + line.unit_price * line.quantity, 0), 0);
...
function Analytics() {
  ...
  const revenue = paidOrders.reduce((total, order) => total + order.items.reduce((sum, item) => sum + item.unit_price * item.quantity, 0), 0);
  const average = paidOrders.length ? Math.round(revenue / paidOrders.length) : 0;
  const productSales = paidOrders.flatMap((order) => order.items).reduce<Record<string, { units: number; revenue: number }>>((result, item) => { const current = result[item.product_name] ?? { units: 0, revenue: 0 }; current.units += item.quantity; current.revenue += item.unit_price * item.quantity; result[item.product_name] = current; return result; }, {});
