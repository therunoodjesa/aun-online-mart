...
type Vendor = { id: string; name: string; is_open: boolean | null; store_type?: 'marketplace' | 'supermarket' | 'service' | null; description?: string | null; banner_url?: string | null; category?: string | null; location?: string | null; pickup_location?: string | null; pickup_instructions?: string | null; important_message?: string | null; average_prep_time?: string | null };
...
export default function VendorPortal() {
  ...
  const load = async () => {
    ...
    const { data: vendorRow } = await supabase.from('vendors').select('id, name, is_open, store_type, description, banner_url, category, location, pickup_location, pickup_instructions, important_message, average_prep_time').eq('owner_id', auth.user.id).maybeSingle();
    if (vendorRow) {
      setVendor(vendorRow as Vendor);
      const [{ data }, { data: serviceRows }, { data: savedSchedule }] = await Promise.all([
        ...
        supabase.from('vendor_schedules').select('weekly_schedule, service_available_dates').eq('vendor_id', vendorRow.id).maybeSingle(),
      ]);
  ...
  const saveSchedule = async () => {
    ...
    const { error } = await supabase.from('vendor_schedules').upsert({ vendor_id: vendor.id, weekly_schedule: schedule, service_available_dates: vendor.store_type === 'service' ? serviceAvailableDates : [], pause_until: null, closed_for_day: null, updated_at: new Date().toISOString() }, { onConflict: 'vendor_id' });
  ...
  const pauseStore = async () => {
    ...
    await setOpen(false);
    const { error } = await supabase.from('vendor_schedules').upsert({ vendor_id: vendor.id, weekly_schedule: schedule, service_available_dates: serviceAvailableDates, pause_until: pauseUntil, closed_for_day: null, updated_at: new Date().toISOString() }, { onConflict: 'vendor_id' });
  ...
  const closeForToday = async () => {
    if (!vendor) return;
    await setOpen(false);
    const { error } = await supabase.from('vendor_schedules').upsert({ vendor_id: vendor.id, weekly_schedule: schedule, service_available_dates: serviceAvailableDates, pause_until: null, closed_for_day: new Date().toISOString().slice(0, 10), updated_at: new Date().toISOString() }, { onConflict: 'vendor_id' });
...
function ServiceCatalogue({ vendor, services, onChanged, setOpen }: { vendor: Vendor; services: BookableService[]; onChanged: () => Promise<void>; setOpen: (value: boolean) => void }) {
  ...
  const toggle = async (service: BookableService) => { const { error } = await supabase.from('services').update({ is_available: !service.is_available, updated_at: new Date().toISOString() }).eq('id', service.id); if (error) { Alert.alert('Could not update service', error.message); return; } await onChanged(); };
...
function Orders({ vendor }: { vendor: Vendor }) {
  ...
  const rejectBooking = async (order: VendorOrder) => {
    ...
    if (error || data?.error) Alert.alert('Could not reject booking', data?.error ?? error?.message ?? 'Please try again.');
...
function VendorApplication() {
  ...
  useEffect(() => { const load = async () => { const { data: auth } = await supabase.auth.getUser(); if (!auth.user) { setLoading(false); return; } setContactName(String(auth.user.user_metadata?.full_name ?? '')); setPhone(String(auth.user.user_metadata?.phone ?? '')); const { data } = await supabase.from('vendor_applications').select('store_name, contact_name, phone, store_type, category, description, address, pickup_location, status').eq('user_id', auth.user.id).maybeSingle(); if (data) { setStoreName(data.store_name); setContactName(data.contact_name); setPhone(data.phone); setStoreType(data.store_type as 'marketplace' | 'supermarket' | 'service'); setCategory(data.category ?? ''); setDescription(data.description ?? ''); setAddress(data.address ?? ''); setPickupLocation(data.pickup_location ?? ''); setSubmitted(data.status === 'pending' || data.status === 'approved'); } setLoading(false); }; void load(); }, []);
  const submit = async () => {
    setFeedback('');
    if (!storeName.trim() || !contactName.trim() || !phone.trim() || category.trim().length < 2) {
      setFeedback('Add the store name, contact name, phone number and customer-facing category.');
      return;
    }
    const { data: auth } = await supabase.auth.getUser();
