...
type PromotionCard = HomePromo & { id?: string; position: number };
type MealPlanRequest = { user_id: string; requested_plan_count: number | null; updated_at: string; profile: { full_name: string | null; phone: string | null; student_id: string | null; age: number | null; school_year: string | null } | null };
type DashboardData = { metrics: { pending_transfers: number; pending_vendor_applications: number; pending_meal_plan_requests: number; paid_orders: number; pending_payouts: number; dispatch_queue: number; gross_sales: number; sales_last_30_days: number; average_order_value: number; partner_vendors: number; top_vendors: { id: string; name: string; sales: number; orders: number }[] }; pending_transfers: Transfer[]; pending_vendor_applications: Application[]; pending_meal_plan_requests: MealPlanRequest[]; pending_payouts: Payout[]; dispatch_queue: DispatchOrder[]; delivery_riders: Rider[]; orders: AdminOrder[]; home_promo: HomePromo | null };
...
type Page = 'overview' | 'orders' | 'transfers' | 'vendors' | 'payouts' | 'dispatch' | 'home' | 'activity' | 'support' | 'wallet';
...
export default function AdminPortal() {
  ...
  const reviewMealPlan = async (request: MealPlanRequest, decision: 'approved' | 'declined') => {
    setWorkingId(request.user_id);
    try {
      await invoke({ action: 'review_meal_plan', user_id: request.user_id, decision });
      const name = request.profile?.full_name?.trim() || 'The student';
      setFeedback(decision === 'approved' ? `${name}'s meal plan was approved.` : `${name}'s meal-plan request was declined.`);
      await load();
    } catch (error) { setFeedback(error instanceof Error ? error.message : 'Could not review that meal-plan request.'); }
    finally { setWorkingId(''); }
  };
  ...
  const creditWallet = async (customerId: string, amount: string, kind: 'refund_credit' | 'goodwill_credit' | 'promotion_credit', description: string) => {
    setWalletSubmitting(true);
    try {
      const result = await invoke({ action: 'credit_wallet', user_id: customerId.trim(), amount: Number(amount), kind, description: description.trim() });
      setFeedback(`₦${Number(result.amount).toLocaleString('en-NG')} AOM Credit was added for ${result.customer_name}.`);
      return true;
    } catch (error) { setFeedback(error instanceof Error ? error.message : 'Could not add AOM Credit.'); return false; }
...
function WalletCreditPanel({ onSubmit, submitting }: { onSubmit: (customerId: string, amount: string, kind: 'refund_credit' | 'goodwill_credit' | 'promotion_credit', description: string) => Promise<boolean>; submitting: boolean }) {
  const [customerId, setCustomerId] = useState('');
  const [amount, setAmount] = useState('');
  const [description, setDescription] = useState('');
  const [kind, setKind] = useState<'refund_credit' | 'goodwill_credit' | 'promotion_credit'>('goodwill_credit');
  const save = async () => {
    const saved = await onSubmit(customerId, amount, kind, description);
    if (saved) { setCustomerId(''); setAmount(''); setDescription(''); }
  };
  return <View style={styles.walletAdminPanel}><View style={styles.walletAdminHeading}><View style={styles.walletAdminIcon}><Ionicons name="wallet-outline" size={23} color="#176E73" /></View><View><Text style={styles.walletAdminTitle}>Issue AOM Credit</Text><Text style={styles.walletAdminSub}>For refunds, goodwill adjustments, or promotional rewards. Credit cannot be withdrawn as cash.</Text></View></View><View style={styles.walletAdminFields}><TextInput value={customerId} onChangeText={setCustomerId} autoCapitalize="none" placeholder="Customer user ID" placeholderTextColor="#8A96A3" style={[styles.walletInput, styles.walletIdInput]} /><TextInput value={amount} onChangeText={setAmount} keyboardType="decimal-pad" placeholder="Amount (₦)" placeholderTextColor="#8A96A3" style={[styles.walletInput, styles.walletAmountInput]} /></View><View style={styles.walletKindRow}>{([['refund_credit', 'Refund'], ['goodwill_credit', 'Goodwill'], ['promotion_credit', 'Promotion']] as const).map(([value, title]) => <TouchableOpacity key={value} onPress={() => setKind(value)} style={[styles.walletKind, kind === value && styles.walletKindActive]}><Text style={[styles.walletKindText, kind === value && styles.walletKindTextActive]}>{title}</Text></TouchableOpacity>)}</View><TextInput value={description} onChangeText={setDescription} placeholder="Reason visible to the customer, e.g. Delivery adjustment for AOM-1042" placeholderTextColor="#8A96A3" style={styles.walletDescription} maxLength={240} /><TouchableOpacity disabled={submitting} onPress={() => void save()} style={[styles.walletSubmit, submitting && { opacity: .6 }]}>{submitting ? <ActivityIndicator color="#01193D" /> : <><Ionicons name="add-circle-outline" size={19} color="#01193D" /><Text style={styles.walletSubmitText}>ADD AOM CREDIT</Text></>}</TouchableOpacity></View>;
}
function Overview({ metrics, transfers, applications, mealPlanRequests, payouts, workingId, onReviewMealPlan, setPage }: { metrics: DashboardData['metrics']; transfers: Transfer[]; applications: Application[]; mealPlanRequests: MealPlanRequest[]; payouts: Payout[]; workingId: string; onReviewMealPlan: (request: MealPlanRequest, decision: 'approved' | 'declined') => Promise<void>; setPage: (page: Page) => void }) {
  ...
  return <>
    ...
    <View style={styles.metricRow}>
      ...
      <Metric icon="card-outline" label="Meal plans to verify" value={metrics.pending_meal_plan_requests} colour="#365B95" />
    ...
    <View style={styles.overviewGrid}>
      ...
      <View style={styles.panel}>
        <View style={styles.panelHead}><Text style={styles.panelTitle}>Pending meal-plan verifications</Text><Text style={styles.panelAction}>{mealPlanRequests.length} waiting</Text></View>
        <Text style={styles.panelCopy}>Approve only after the student’s submitted details are verified.</Text>
        {mealPlanRequests.slice(0, 4).map((request) => <View style={styles.previewRow} key={request.user_id}><View style={{ flex: 1 }}><Text style={styles.previewTitle}>{request.profile?.full_name?.trim() || 'Student'}</Text><Text style={styles.previewSub}>{request.profile?.student_id || 'Student ID not provided'} · {request.requested_plan_count ?? 0} plan{request.requested_plan_count === 1 ? '' : 's'}</Text>{request.profile?.phone ? <Text style={styles.previewSub}>{request.profile.phone}</Text> : null}</View><View style={{ flexDirection: 'row', gap: 7 }}><TouchableOpacity disabled={workingId === request.user_id} onPress={() => void onReviewMealPlan(request, 'declined')} style={[styles.payoutReject, workingId === request.user_id && { opacity: 0.5 }]}><Text style={styles.payoutRejectText}>Decline</Text></TouchableOpacity><TouchableOpacity disabled={workingId === request.user_id} onPress={() => void onReviewMealPlan(request, 'approved')} style={[styles.confirmButton, workingId === request.user_id && { opacity: 0.5 }]}>{workingId === request.user_id ? <ActivityIndicator size="small" color="#01193D" /> : <Text style={styles.confirmText}>Approve</Text>}</TouchableOpacity></View></View>)}
        {!mealPlanRequests.length ? <Empty text="No meal-plan verifications await review." /> : null}
      </View>
    </View>
  </>;
}
...
function HomePromotionEditor() {
  const blank = (position: number): PromotionCard => ({ position, heading: "TODAY'S PICK", message: '', background_image_url: null, background_color: '#01193D', cta_label: 'ORDER NOW', cta_href: '/(buyer)/marketplace/category/meals' });
  ...
  useEffect(() => { const load = async () => { const { data } = await supabase.from('home_promotion_cards').select('id, position, heading, message, background_image_url, background_color, cta_label, cta_href').order('position'); setPromos((data?.length ? data : [blank(1)]) as PromotionCard[]); setLoading(false); }; void load(); }, []);
  const update = (position: number, patch: Partial<PromotionCard>) => setPromos((current) => current.map((promo) => promo.position === position ? { ...promo, ...patch } : promo));
  const add = () => setPromos((current) => current.length >= 3 ? current : [...current, blank(current.length + 1)]);
  const remove = (position: number) => setPromos((current) => current.filter((promo) => promo.position !== position).map((promo, index) => ({ ...promo, position: index + 1 })));
  const save = async () => { setSaving(true); setFeedback(''); const { data, error } = await supabase.functions.invoke('admin-portal', { body: { action: 'update_home_promos', promotions: promos } }); setSaving(false); if (error || data?.error) { setFeedback(data?.error ?? error?.message ?? 'Could not update Today’s Pick.'); return; } setPromos(data.home_promos as PromotionCard[]); setFeedback('Today’s Pick cards are live on the buyer home page.'); };
  if (loading) return <ActivityIndicator color="#176E73" style={{ marginVertical: 24 }} />;
  return <View style={{ marginTop: 16, paddingTop: 18, borderTopWidth: 1, borderTopColor: '#E5EBEF' }}><Text style={{ color: '#01193D', fontSize: 17, fontWeight: '800' }}>Today’s Pick cards</Text><Text style={{ color: '#7B8794', fontSize: 13, lineHeight: 18, marginTop: 5, marginBottom: 14 }}>Create up to three promotions. Customers swipe across them on the home page.</Text>{promos.map((promo, index) => <View key={promo.id ?? promo.position} style={{ borderWidth: 1, borderColor: '#DDE5EB', borderRadius: 10, padding: 13, marginBottom: 12 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><Text style={{ color: '#176E73', fontSize: 13, fontWeight: '800' }}>CARD {index + 1}</Text>{promos.length > 1 ? <TouchableOpacity onPress={() => remove(promo.position)}><Text style={{ color: '#B34A4A', fontSize: 12, fontWeight: '800' }}>Remove</Text></TouchableOpacity> : null}</View><View style={{ flexDirection: 'row', gap: 12, marginTop: 10 }}><View style={{ flex: 1 }}><PromoField label="Heading" value={promo.heading} onChangeText={(heading) => update(promo.position, { heading })} /></View><View style={{ width: 150 }}><PromoField label="Button label" value={promo.cta_label} onChangeText={(cta_label) => update(promo.position, { cta_label })} /></View></View><PromoField label="Message" value={promo.message} onChangeText={(message) => update(promo.position, { message })} multiline /><View style={{ flexDirection: 'row', gap: 12 }}><View style={{ flex: 1 }}><PromoField label="Background image URL" value={promo.background_image_url ?? ''} onChangeText={(background_image_url) => update(promo.position, { background_image_url: background_image_url || null })} autoCapitalize="none" /></View><View style={{ width: 150 }}><PromoField label="Background colour" value={promo.background_color} onChangeText={(background_color) => update(promo.position, { background_color })} autoCapitalize="none" /></View></View><PromoField label="Button destination" value={promo.cta_href} onChangeText={(cta_href) => update(promo.position, { cta_href })} autoCapitalize="none" /></View>)}<View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>{promos.length < 3 ? <TouchableOpacity onPress={add} style={{ minHeight: 40, borderWidth: 1, borderColor: '#176E73', borderRadius: 8, paddingHorizontal: 13, justifyContent: 'center' }}><Text style={{ color: '#176E73', fontSize: 12, fontWeight: '800' }}>+ Add card</Text></TouchableOpacity> : <Text style={{ color: '#7B8794', fontSize: 12 }}>Maximum of 3 cards</Text>}<TouchableOpacity disabled={saving} onPress={() => void save()} style={{ minHeight: 42, borderRadius: 8, backgroundColor: '#68ECCB', paddingHorizontal: 16, justifyContent: 'center' }}>{saving ? <ActivityIndicator color="#01193D" /> : <Text style={{ color: '#01193D', fontSize: 13, fontWeight: '800' }}>Save cards</Text>}</TouchableOpacity></View>{feedback ? <Text style={{ color: feedback.includes('live') ? '#176E73' : '#B34A4A', fontSize: 13, fontWeight: '700', marginTop: 10 }}>{feedback}</Text> : null}</View>;
}
...
Object.assign(styles, {
  walletAdminPanel: { ...styles.panel, marginTop: 22, maxWidth: 780 },
  walletAdminHeading: { flexDirection: 'row', gap: 12, alignItems: 'flex-start', marginBottom: 18 },
  walletAdminIcon: { width: 44, height: 44, borderRadius: 12, backgroundColor: '#E1F5EE', alignItems: 'center', justifyContent: 'center' },
  walletAdminTitle: { color: '#01193D', fontSize: 19, fontWeight: '800' },
  walletAdminSub: { color: '#657283', fontSize: 13, lineHeight: 19, marginTop: 3, maxWidth: 550 },
  walletAdminFields: { flexDirection: 'row', gap: 10 },
  walletInput: { height: 46, borderWidth: 1, borderColor: '#C9D4DF', borderRadius: 8, backgroundColor: '#FFFFFF', paddingHorizontal: 12, color: '#01193D', fontSize: 14 },
  walletIdInput: { flex: 1 }, walletAmountInput: { width: 145 },
  walletKindRow: { flexDirection: 'row', gap: 8, marginTop: 11 },
  walletKind: { paddingHorizontal: 12, height: 36, borderRadius: 18, borderWidth: 1, borderColor: '#C9D4DF', alignItems: 'center', justifyContent: 'center' },
  walletKindActive: { backgroundColor: '#E1F5EE', borderColor: '#25B68A' },
  walletKindText: { color: '#647181', fontSize: 13, fontWeight: '700' }, walletKindTextActive: { color: '#176E73' },
  walletDescription: { minHeight: 72, marginTop: 11, borderWidth: 1, borderColor: '#C9D4DF', borderRadius: 8, padding: 12, color: '#01193D', fontSize: 14, textAlignVertical: 'top' },
  walletSubmit: { alignSelf: 'flex-start', minHeight: 43, paddingHorizontal: 15, borderRadius: 8, backgroundColor: '#68ECCB', flexDirection: 'row', alignItems: 'center', gap: 7, justifyContent: 'center', marginTop: 12 },
  walletSubmitText: { color: '#01193D', fontWeight: '800', fontSize: 13 },
