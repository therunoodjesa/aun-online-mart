...
type CategoryProduct = { id: string; vendor_id: string; name: string; price: number; image_url: string | null; marketplace_subcategory: string | null; category: string | null };
...
export default function MarketplaceCategoryPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      setLoading(true);
      const fields = 'id, vendor_id, name, price, image_url, marketplace_subcategory, category';
      const [{ data, error }, { data: placementRows }] = await Promise.all([
        supabase.from('products').select(fields).eq('marketplace_category', key).eq('status', 'available'),
      ...
      const { data: placedProducts } = placementIds.length ? await supabase.from('products').select(fields).in('id', placementIds).eq('status', 'available') : { data: [] as CategoryProduct[] };
      if (error) {
        const legacy = await supabase.from('products').select(fields).eq('category', config.title).eq('status', 'available');
