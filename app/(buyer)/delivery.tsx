...
type SavedLocation = { address: string; directions: string };
type LocationSuggestion = SavedLocation & { source: 'last-used' | 'campus' | 'adamawa' };
...
export default function DeliveryPage() {
  ...
  const [lastLocation, setLastLocation] = useState<SavedLocation | null>(null);
  ...
  useEffect(() => {
    ...
    const loadLastLocation = async () => {
      ...
      const key = `aom:last-delivery-location:${auth.user?.id ?? 'guest'}`;
      const saved = await AsyncStorage.getItem(key);
      if (!saved || !active) return;
      try {
        const parsed = JSON.parse(saved) as SavedLocation;
        if (!parsed.address?.trim()) return;
        setLastLocation({ address: parsed.address.trim(), directions: parsed.directions?.trim() ?? '' });
        setAddress((current) => current || parsed.address.trim());
        setDirections((current) => current || parsed.directions?.trim() || '');
      } catch { /* Ignore a malformed old local value. */ }
    };
    void loadLastLocation();
    return () => { active = false; };
  }, []);

  const rememberLocation = async (nextAddress: string, nextDirections: string) => {
    const cleanedAddress = nextAddress.trim();
    if (!cleanedAddress) return;
    const saved = { address: cleanedAddress, directions: nextDirections.trim() };
    setLastLocation(saved);
    const { data: auth } = await supabase.auth.getUser();
    await AsyncStorage.setItem(`aom:last-delivery-location:${auth.user?.id ?? 'guest'}`, JSON.stringify(saved));
  };

  const addressSuggestions = useMemo(() => {
    const query = address.trim().toLowerCase();
    const choices: LocationSuggestion[] = [
      ...(lastLocation ? [{ ...lastLocation, source: 'last-used' as const }] : []),
      ...deliveryLocationSuggestions.map((value) => ({ address: value, directions: '', source: 'campus' as const })),
      ...adamawaLocationSuggestions.map((value) => ({ address: value, directions: '', source: 'adamawa' as const })),
    ];
    const seen = new Set<string>();
    return choices.filter((choice) => {
      const key = choice.address.toLowerCase();
  ...
  return <View style={styles.screen}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      ...
      <View style={styles.panel}>
        <View style={styles.addressCard}>{editingAddress ? <View style={styles.addressEditor}><View style={styles.addressEditorHeader}><Text style={styles.cardLabel}>DELIVERY ADDRESS</Text><TouchableOpacity onPress={() => { void rememberLocation(address, directions); setEditingAddress(false); }}><Text style={styles.edit}>Done</Text></TouchableOpacity></View><TextInput value={address} onChangeText={(value) => { setAddress(value); setShowSuggestions(true); }} onFocus={() => setShowSuggestions(true)} placeholder="Dorm, hall, or location" placeholderTextColor="#7E7E7E" style={styles.addressInput} />{showSuggestions && <View style={styles.suggestions}>{addressSuggestions.map((suggestion) => <TouchableOpacity key={`${suggestion.source}-${suggestion.address}`} style={styles.suggestion} onPress={() => { setAddress(suggestion.address); if (suggestion.directions) setDirections(suggestion.directions); setShowSuggestions(false); }}><Ionicons name={suggestion.source === 'last-used' ? 'time-outline' : 'location-outline'} size={16} color="#175E63" /><View style={{ flex: 1 }}><Text style={styles.suggestionText}>{suggestion.address}</Text><Text style={styles.suggestionSource}>{suggestion.source === 'last-used' ? 'Last used location' : suggestion.source === 'campus' ? 'AUN campus' : 'Adamawa location'}</Text></View></TouchableOpacity>)}{!addressSuggestions.length ? <Text style={styles.noSuggestions}>No saved suggestion yet — enter your exact location.</Text> : null}</View>}<TextInput value={directions} onChangeText={setDirections} placeholder="Room number or further directions (optional)" placeholderTextColor="#7E7E7E" multiline style={[styles.addressInput, styles.directionsInput]} /></View> : <><View style={styles.addressIcon}><Ionicons name="home-outline" size={24} color="#F8F3ED" /></View><View style={styles.addressCopy}><Text style={styles.cardLabel}>DELIVERY ADDRESS</Text><Text style={styles.addressName}>{address || 'Add a location'}</Text><Text numberOfLines={1} style={styles.addressDetail}>{directions || 'Dorm name, room, or directions'}</Text></View><TouchableOpacity onPress={() => setEditingAddress(true)}><Text style={styles.edit}>Edit</Text></TouchableOpacity></>}</View>
        ...
        {serverQuote?.campusDelivery?.active && <View style={styles.campusRate}><Ionicons name="location-outline" size={18} color="#175E63" /><Text style={styles.campusRateText}>₦800 campus delivery applied</Text></View>}
