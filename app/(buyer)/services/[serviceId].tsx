...
type BookingChoice = { id: string; serviceId: string; name: string; duration: string; price: number };
...
export default function ServiceBookingPage() {
  ...
  useEffect(() => { if (!serviceId) return; void (async () => {
    ...
    const [{ data: rows }, { data: schedule }] = await Promise.all([
      ...
      supabase.from('vendor_schedules').select('service_available_dates').eq('vendor_id', vendor.id).maybeSingle(),
    ]);
    ...
    const options: BookingChoice[] = serviceRows.flatMap((row) => {
      ...
      return choices.length ? choices.map((choice) => ({ id: choice.id, serviceId: row.id, name: choice.name, duration: `${choice.duration_minutes ?? row.duration_minutes ?? 60} minutes`, price: Number(choice.price) })) : [{ id: `${row.id}-standard`, serviceId: row.id, name: row.name, duration: `${row.duration_minutes ?? 60} minutes`, price: Number(row.starting_price ?? 0) }];
    });
