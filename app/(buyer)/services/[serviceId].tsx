...
import type { BookingService } from '../../../lib/service-fallback';
...
export default function ServiceBookingPage() {
  ...
  useEffect(() => { if (!serviceId) return; void (async () => {
    ...
    const { data: optionRows } = await supabase.from('service_options').select('id, name, price, duration_minutes, option_type').eq('service_id', row.id).eq('is_available', true).order('sort_order').order('name');
    const options = (optionRows ?? []).filter((option) => option.option_type !== 'addon').map((option) => ({ id: option.id, name: option.name, duration: `${option.duration_minutes ?? row.duration_minutes ?? 60} minutes`, price: Number(option.price) }));
    const addons = (optionRows ?? []).filter((option) => option.option_type === 'addon').map((option) => ({ id: option.id, name: option.name, price: Number(option.price) }));
    ...
    setVendorId(row.vendor_id ?? null);
    setService({ id: row.id, name: row.name ?? 'Service provider', category: row.category ?? 'Service', description: row.description ?? '', price: Number(row.starting_price ?? 0), imageUrl: row.image_url ?? null, rating: 4.9, duration: `${row.duration_minutes ?? 60} mins`, options: options.length ? options : [defaultOption], addons });
  ...
  const chosenAddons = service.addons.filter((addon) => selectedAddons.includes(addon.id));
  ...
  const proceed = async () => {
    ...
    addItem({ productId: `service:${serviceId}:${chosen.id}:${dateKey(selectedDate)}:${addonKey}`, name: chosen.name, category: `${service.name} · ${selectedDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`, price: bookingPrice, imageUrl: service.imageUrl, note: [addonSummary, note.trim()].filter(Boolean).join(' · ') || null });
