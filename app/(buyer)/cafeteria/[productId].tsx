...
export default function CafeteriaProductPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: recommendationIds } = data ? await supabase.rpc('cafeteria_recommendations', { p_product_id: productId, p_limit: 8 }) : { data: [] };
      const ids = (recommendationIds ?? []).map((row: { product_id: string }) => row.product_id);
      const { data: relatedRows } = ids.length ? await supabase.from('cafeteria_products').select('id, name, description, category, categories, sort_order, price, image_url, status, meal_plan_eligible').in('id', ids) : { data: [] };
...
const sleek = StyleSheet.create({
  relatedSection: { marginTop: 30 },
  relatedTitle: { color: '#01193D', fontSize: 21, fontWeight: '800', marginBottom: 14 },
  relatedRow: { gap: 13, paddingRight: 20 },
  relatedCard: { width: 154, borderRadius: 14, overflow: 'hidden', backgroundColor: '#F2F4F7', paddingBottom: 11 },
  relatedImage: { width: '100%', height: 128, backgroundColor: '#DDE3EB', resizeMode: 'cover' },
  relatedName: { color: '#01193D', fontSize: 14, fontWeight: '700', marginHorizontal: 10, marginTop: 9 },
  relatedPrice: { color: '#176E73', fontSize: 13, fontWeight: '800', marginHorizontal: 10, marginTop: 4 },
});
