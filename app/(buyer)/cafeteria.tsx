...
export default function CafeteriaPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data }, { data: settingRow }] = await Promise.all([
        supabase.from('cafeteria_products').select('id, name, description, category, categories, sort_order, price, image_url, status, meal_plan_eligible').contains('categories', [category]).neq('status', 'hidden').order('sort_order').order('name'),
      ...
      if (active) { setProducts((data ?? []) as Product[]); if (settingRow) setSettings(settingRow as CafeteriaSettings); setLoading(false); }
    };
    void load();
    return () => { active = false; };
  }, [category]);
  ...
  const serviceOpen = settings.is_accepting_orders && settings[`${category}_open`];
  ...
  return <View style={styles.screen}>
    ...
    <FlatList
      ...
      ListHeaderComponent={<>
        ...
        <View style={[styles.note, !serviceOpen && styles.closedNote]}><Text style={[styles.noteText, !serviceOpen && styles.closedNoteText]}>{!settings.is_accepting_orders ? (settings.customer_notice || 'Cafeteria ordering is temporarily paused. Please check back shortly.') : !settings[`${category}_open`] ? `${currentTab.label} ordering is closed right now.` : (settings.customer_notice || currentTab.note)}</Text></View>
      </>}
      renderItem={({ item }) => <TouchableOpacity activeOpacity={0.9} onPress={() => openProduct(item)} style={[styles.card, { width: cardWidth }]}><View style={styles.photoWrap}>{item.image_url ? <Image source={{ uri: item.image_url }} style={styles.photo} /> : <View style={styles.photoPlaceholder}><Ionicons name="fast-food-outline" size={38} color={COLORS.muted} /></View>}{(item.status === 'sold_out' || !serviceOpen) && <View style={styles.sold}><Text style={styles.soldText}>{item.status === 'sold_out' ? 'SOLD OUT' : 'CLOSED'}</Text></View>}</View><View style={styles.cardInfo}><View style={styles.nameRow}><Text numberOfLines={1} style={styles.productName}>{item.name}</Text><TouchableOpacity onPress={() => void addOrCustomise(item)} disabled={item.status === 'sold_out' || !serviceOpen}><Ionicons name="cart-outline" size={21} color={COLORS.cream} /></TouchableOpacity></View><View style={styles.purchaseRow}>{item.status === 'available' && serviceOpen ? <View style={styles.stepper}><TouchableOpacity onPress={() => update(item, -1)}><Text style={styles.step}>−</Text></TouchableOpacity><Text style={styles.quantity}>{quantity(item.id)}</Text><TouchableOpacity onPress={() => void addOrCustomise(item)}><Text style={styles.step}>+</Text></TouchableOpacity></View> : <Text style={styles.customise}>{item.status === 'sold_out' ? 'Sold out' : 'Closed'}</Text>}<Text style={styles.price}>{money(item.price)}</Text></View></View></TouchableOpacity>}
