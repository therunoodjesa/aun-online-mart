  ...
  return <View style={styles.screen}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      ...
      {isSupermarket ? <View><SupermarketStores vendors={supermarketVendors} width={supermarketVendorWidth} onPress={(vendorId) => router.push({ pathname: '/(buyer)/marketplace/[vendorId]', params: { vendorId } })} /><View style={styles.supermarketGrid}>{SUPERMARKET.map((item) => <TouchableOpacity key={item.label} style={styles.supermarketTileWrap} activeOpacity={0.88} onPress={() => router.push({ pathname: '/(buyer)/supermarket/[category]', params: { category: item.slug } })}><ImageBackground source={item.image} imageStyle={styles.tileImage} style={styles.supermarketTile}><View style={styles.tileShade} /><Text style={styles.tileLabel}>{item.label}</Text></ImageBackground></TouchableOpacity>)}</View> </View> : <View style={styles.body}>
        ...
        <FlatList horizontal data={homePromos.length ? homePromos : [{ id: 'fallback', position: 1, heading: "TODAY'S PICK", message: "Sholly's jollof is extra smoky today.", background_image_url: null, background_color: COLORS.navy, cta_label: 'ORDER NOW', cta_href: '/(buyer)/marketplace/category/meals' }]} showsHorizontalScrollIndicator={false} keyExtractor={(item) => item.id} contentContainerStyle={styles.promoRail} renderItem={({ item }) => <ImageBackground source={item.background_image_url ? { uri: item.background_image_url } : PROMO_IMAGE} imageStyle={styles.promoImage} style={[styles.promo, { backgroundColor: item.background_color || COLORS.navy }]}><View style={styles.promoOverlay} /><Text style={styles.promoEyebrow}>{item.heading}</Text><Text style={styles.promoTitle}>{item.message}</Text><TouchableOpacity style={styles.orderButton} onPress={() => openHomePromo(item)}><Text style={styles.orderButtonText}>{item.cta_label}</Text></TouchableOpacity></ImageBackground>} />
