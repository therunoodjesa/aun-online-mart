...
type HomePromo = { id: string; position: number; heading: string; message: string; background_image_url: string | null; background_color: string; cta_label: string; cta_href: string };
...
function SectionTitle({ title, onSeeAll }: { title: string; onSeeAll?: () => void }) {
  return <View style={styles.sectionTitleRow}><Text style={styles.sectionTitle}>{title}</Text>{onSeeAll ? <TouchableOpacity onPress={onSeeAll} accessibilityRole="button" accessibilityLabel={`See all ${title.toLowerCase()}`}><Text style={styles.seeAll}>See all</Text></TouchableOpacity> : null}</View>;
}
...
function HomePromoCard({ promo, onPress }: { promo: HomePromo; onPress: () => void }) {
  const content = <><Text style={styles.promoEyebrow}>{promo.heading}</Text><Text style={styles.promoTitle}>{promo.message}</Text><TouchableOpacity style={styles.orderButton} onPress={onPress}><Text style={styles.orderButtonText}>{promo.cta_label}</Text></TouchableOpacity></>;
  const backgroundColor = /^#[0-9A-Fa-f]{6}$/.test(promo.background_color ?? '') ? promo.background_color : COLORS.navy;
  if (!promo.background_image_url) return <View style={[styles.promo, { backgroundColor }]}>{content}</View>;
  return <ImageBackground source={{ uri: promo.background_image_url }} imageStyle={styles.promoImage} style={[styles.promo, { backgroundColor }]}><View style={styles.promoOverlay} />{content}</ImageBackground>;
}
...
export default function BuyerHome() {
  ...
  const [homePromos, setHomePromos] = useState<HomePromo[]>([]);
  ...
  useEffect(() => {
    const loadHomePromos = async () => {
      const { data } = await supabase.from('home_promotion_cards').select('id, position, heading, message, background_image_url, background_color, cta_label, cta_href').order('position');
      if (data?.length) setHomePromos(data as HomePromo[]);
    };
    void loadHomePromos();
  }, []);
  ...
  const openHomePromo = (promo: HomePromo) => {
    const destination = promo.cta_href?.trim();
  ...
  return <View style={styles.screen}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      ...
      {isSupermarket ? <View><SupermarketStores vendors={supermarketVendors} width={supermarketVendorWidth} onPress={(vendorId) => router.push({ pathname: '/(buyer)/marketplace/[vendorId]', params: { vendorId } })} /><View style={styles.supermarketGrid}>{SUPERMARKET.map((item) => <TouchableOpacity key={item.label} style={styles.supermarketTileWrap} activeOpacity={0.88} onPress={() => router.push({ pathname: '/(buyer)/supermarket/[category]', params: { category: item.slug } })}><ImageBackground source={item.image} imageStyle={styles.tileImage} style={styles.supermarketTile}><View style={styles.tileShade} /><Text style={styles.tileLabel}>{item.label}</Text></ImageBackground></TouchableOpacity>)}</View> </View> : <View style={styles.body}>
        ...
        <FlatList horizontal data={homePromos.length ? homePromos : [{ id: 'fallback', position: 1, heading: "TODAY'S PICK", message: "Sholly's jollof is extra smoky today.", background_image_url: null, background_color: COLORS.navy, cta_label: 'ORDER NOW', cta_href: '/(buyer)/marketplace/category/meals' }]} showsHorizontalScrollIndicator={false} keyExtractor={(item) => item.id} contentContainerStyle={styles.promoRail} renderItem={({ item }) => <HomePromoCard promo={item} onPress={() => openHomePromo(item)} />} />
        <SectionTitle title="Vendors open now" onSeeAll={() => router.push('/(buyer)/marketplace/vendors')} />
