...
type SavedVendor = { id: string; name: string; category: string | null; imageUrl: string | null; location: string | null; isOpen: boolean | null; storeType?: 'marketplace' | 'supermarket' | 'service' | null };
...
export default function FavouritesPage() {
  ...
  const load = async () => {
    ...
    const [{ data: products }, { data: cafeteriaProducts }, { data: vendorRows }] = await Promise.all([
      ...
      vendorIds.length ? supabase.from('vendors').select('id, name, category, banner_url, location, is_open, store_type').in('id', vendorIds) : Promise.resolve({ data: [] }),
    ]);
    ...
    const vendorMap = new Map(resolvedVendors.map((item: any) => [item.id, { id: item.id, name: item.name, category: item.category, imageUrl: item.banner_url, location: item.location, isOpen: item.is_open, storeType: item.store_type }]));
