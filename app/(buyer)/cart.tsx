...
type ServerQuote = { subtotal: number; serviceFee: number; packagingFee: number; mealPlanCredit: number; deliveryFee: number; total: number; promoCode?: string | null; promoDiscount?: number; campusDelivery?: { active?: boolean } };

function prepMinutes(value: string | null | undefined) {
  const values = (value?.match(/\d+(?:\.\d+)?/g) ?? []).map(Number).filter(Number.isFinite);
  if (!values.length) return null;
  return { low: values[0], high: values[values.length - 1] };
}

function dispatchEstimate(prepTimes: Array<string | null | undefined>) {
  const ranges = prepTimes.map(prepMinutes).filter((range): range is { low: number; high: number } => Boolean(range));
  if (!ranges.length) return 'Est. 45 minutes';
  // An order can only leave after its slowest store is ready. Allow 15–20 minutes for dispatch.
  const low = Math.max(...ranges.map((range) => range.low)) + 15;
  const high = Math.max(...ranges.map((range) => range.high)) + 20;
  return low === high ? `Est. ~${high} minutes` : `Est. ${low}–${high} minutes`;
}
...
export default function CartPage() {
  ...
  const [dispatchTime, setDispatchTime] = useState('Est. 45 minutes');
  const needsServerQuote = items.length > 0 && items.every((item) => !item.productId.startsWith('service:'));
  const quotePending = needsServerQuote && !serverQuote;
  const itemSubtotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const subtotal = serverQuote?.subtotal ?? itemSubtotal;
  const deliveryFee = delivery === 'dispatch' ? (serverQuote?.deliveryFee ?? 0) : 0;
  const serviceFee = serverQuote?.serviceFee ?? 0;
  const packagingFee = serverQuote?.packagingFee ?? 0;
  const mealPlanCredit = serverQuote?.mealPlanCredit ?? 0;
  const total = serverQuote?.total ?? itemSubtotal;
  const cafeteriaEligible = items.some((item) => item.category?.toLowerCase().startsWith('cafeteria') && item.mealPlanEligible !== false);
  ...
  useEffect(() => {
    let active = true;
    const loadDispatchEstimate = async () => {
      const productIds = [...new Set(items
        .map((item) => item.productId.split(':')[0])
        .filter((id) => id.length > 20))];

      if (!productIds.length) {
        if (active) setDispatchTime(hasCafeteria ? 'Est. 15–40 minutes' : 'Est. 45 minutes');
        return;
      }
      ...
      const { data: vendors } = await supabase.from('vendors').select('average_prep_time').in('id', vendorIds);
      if (active) setDispatchTime(dispatchEstimate((vendors ?? []).map((vendor) => vendor.average_prep_time)));
    };
    void loadDispatchEstimate();
    return () => { active = false; };
  }, [items, hasCafeteria]);
  ...
  useEffect(() => {
    ...
    const loadQuote = async () => {
      ...
      const { data, error } = await supabase.functions.invoke('checkout-quote', { body: { items: quoteItems, fulfilment: delivery === 'pickup' ? 'pickup' : 'delivery', use_meal_plan: useMealPlan, promo_code: appliedPromo || null } });
      if (!active) return;
      if (!error && data?.pricing) {
        setServerQuote(data.pricing as ServerQuote);
        if (appliedPromo) setPromoMessage(`${data.pricing.promoCode} applied — you save ₦${Number(data.pricing.promoDiscount ?? 0).toLocaleString('en-NG')}.`);
      } else {
        setServerQuote(null);
        if (appliedPromo) { setAppliedPromo(''); setPromoMessage(data?.error ?? 'That promo code could not be applied.'); }
      }
    };
    void loadQuote();
    return () => { active = false; };
  }, [items, delivery, useMealPlan, appliedPromo]);
  ...
  return <View style={styles.screen}>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {serviceFee > 0 && <View style={{ marginBottom: 14, borderRadius: 8, padding: 12, backgroundColor: '#E1F5EE', flexDirection: 'row', justifyContent: 'space-between' }}><Text style={{ color: '#175E63', fontSize: 14, fontWeight: '700' }}>AOM service fee</Text><Text style={{ color: '#175E63', fontSize: 14, fontWeight: '800' }}>₦ {serviceFee.toLocaleString('en-NG')}</Text></View>}
      {items.length === 0 ? <View style={styles.empty}><Ionicons name="cart-outline" size={52} color="#A0A0A0" /><Text style={styles.emptyTitle}>Your cart is empty</Text><TouchableOpacity style={styles.browseButton} onPress={() => router.back()}><Text style={styles.browseText}>Browse marketplace</Text></TouchableOpacity></View> : <>
        ...
        <View style={styles.promo}><TextInput autoCapitalize="characters" value={promo} onChangeText={(value) => { setPromo(value); if (appliedPromo && value.trim().toUpperCase() !== appliedPromo) { setAppliedPromo(''); setPromoMessage(''); } }} placeholder="Enter promo code" placeholderTextColor="#7E7E7E" style={styles.promoInput} /><TouchableOpacity style={styles.apply} onPress={() => { const code = promo.trim().toUpperCase(); if (!code) { Alert.alert('Enter a promo code', 'Type your code first, then tap Apply.'); return; } setPromo(code); setPromoMessage('Checking your code…'); setAppliedPromo(code); }}><Text style={styles.applyText}>Apply</Text></TouchableOpacity></View>
        {promoMessage ? <Text style={[styles.promoFeedback, serverQuote?.promoCode ? styles.promoSuccess : styles.promoError]}>{promoMessage}</Text> : null}
        {(packagingFee > 0 || mealPlanCredit > 0) && <View style={styles.cafeteriaBreakdown}>{packagingFee > 0 && <Text style={styles.breakdownText}>Cafeteria handling · ₦{packagingFee.toLocaleString('en-NG')}</Text>}{useMealPlan && mealPlanCredit > 0 && <Text style={styles.credit}>Meal-plan credit applied · −₦{mealPlanCredit.toLocaleString('en-NG')}</Text>}</View>}
        <View style={styles.summary}><View style={styles.summaryRow}><Text style={styles.summaryText}>Items subtotal ({items.reduce((sum, item) => sum + item.quantity, 0)} items)</Text><Text style={styles.summaryText}>₦ {subtotal.toLocaleString('en-NG')}</Text></View>{quotePending ? <Text style={styles.calculating}>Calculating your exact checkout total…</Text> : <>{packagingFee > 0 && <View style={styles.summaryRow}><Text style={styles.summaryText}>Cafeteria handling</Text><Text style={styles.summaryText}>₦ {packagingFee.toLocaleString('en-NG')}</Text></View>}{mealPlanCredit > 0 && <View style={styles.summaryRow}><Text style={styles.credit}>Meal-plan credit</Text><Text style={styles.credit}>−₦ {mealPlanCredit.toLocaleString('en-NG')}</Text></View>}{serverQuote?.promoDiscount ? <View style={styles.summaryRow}><Text style={styles.credit}>Promo code ({serverQuote.promoCode})</Text><Text style={styles.credit}>−₦ {Number(serverQuote.promoDiscount).toLocaleString('en-NG')}</Text></View> : null}<View style={styles.summaryRow}><Text style={styles.summaryText}>Delivery fee</Text><Text style={styles.summaryText}>₦ {deliveryFee.toLocaleString('en-NG')}</Text></View><View style={styles.totalRow}><Text style={styles.totalLabel}>AMOUNT TO PAY</Text><Text style={styles.total}>₦ {total.toLocaleString('en-NG')}</Text></View></>}</View>
        <TouchableOpacity disabled={quotePending} style={[styles.proceed, quotePending && styles.proceedDisabled]} onPress={() => { if (hasCafeteria && !hasMealPlanAccount) { router.push({ pathname: '/(buyer)/profile', params: { edit: 'true' } }); return; } const checkoutProperties = { item_count: items.reduce((sum, item) => sum + item.quantity, 0), total, fulfilment: delivery, uses_meal_plan: useMealPlan, promo_code: appliedPromo || undefined }; posthog.capture('checkout_started', checkoutProperties); void recordJourneyEvent('checkout_started', '/cart', checkoutProperties); if (delivery === 'dispatch') router.push({ pathname: '/(buyer)/delivery', params: { mealPlan: useMealPlan ? 'true' : 'false', promo: appliedPromo || undefined } }); else router.push({ pathname: '/(buyer)/payment', params: { fulfilment: 'pickup', mealPlan: useMealPlan ? 'true' : 'false', promo: appliedPromo || undefined } }); }}><Text style={styles.proceedText}>{quotePending ? 'CALCULATING TOTAL…' : hasCafeteria && !hasMealPlanAccount ? 'COMPLETE PROFILE DETAILS' : delivery === 'dispatch' ? 'PROCEED TO DELIVERY' : 'PROCEED TO PAYMENT'}</Text></TouchableOpacity>
      </>}
    </ScrollView>
  </View>;
}
