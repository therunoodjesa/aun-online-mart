...
type Service = { id: string; name: string; category: string; price: number; imageUrl: string | null; available: boolean; providerName: string | null };
...
export default function ServicesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const availableVendors = await applyVendorAvailability((vendors ?? []) as { id: string; name: string; is_approved: boolean; is_open: boolean | null }[]);
      const vendorsById = new Map(availableVendors.map((vendor) => [vendor.id, vendor]));
